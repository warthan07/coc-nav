...
var kindToLabel = (kind) => {
  const config = import_coc.workspace.getConfiguration("suggest").get("completionItemKindLabels");
...
var nav = async () => {
  ...
  const symbols = ((_a = await provider.provideDocumentSymbols(document, tokenSource.token)) != null ? _a : []).map((v) => expandChildren(v)).flat();
  let arr = [];
